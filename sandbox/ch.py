...
PACKAGEDIR = "/Users/ch/repos/tess-point/src/"
pointings = pd.read_csv(f"{PACKAGEDIR}/data/pointings.csv").set_index("sector")
...
class TESSPoint:
    ...
    def __post_init__(self):
        xeul = np.hstack(
            [
                (np.pi / 180.0) * pointings.loc[self.sector]["ra"],
                np.pi / 2.0 - (np.pi / 180.0) * pointings.loc[self.sector]["dec"],
                (np.pi / 180.0) * pointings.loc[self.sector]["roll"] + np.pi,
            ]
        )
